Feature: Telemetry ingestion
  ...
  Background:
    ...
    And drone "DRN-1" is assigned to delivery "DLV-100" in status "InProgress"

  Scenario: A valid telemetry update is accepted and stored
    When drone "DRN-1" reports position "44.50, 11.35" with status "InFlight"
  ...
  Scenario: Telemetry for an unknown drone is rejected
    When an unknown drone "DRN-999" reports position "44.50, 11.35" with status "InFlight"
  ...
  Scenario: Telemetry with an invalid position is rejected
    When drone "DRN-1" reports position "999.0, 999.0" with status "InFlight"
  ...
  Scenario: Telemetry for an idle drone updates fleet state without affecting any delivery
    ...
    When drone "DRN-2" reports position "44.49, 11.34" with status "Idle"
    Then the update should be accepted
