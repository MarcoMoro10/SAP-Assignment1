Feature: Create a delivery request
  ...
  I want to create a delivery request specifying package, pickup location and destination
  ...
  Scenario: Successful scheduled delivery creation
    When I create a delivery with weight "2" kg, pickup location "via Emilia, 9", destination "via Veneto, 5" to ship on "2026-06-10" at "10:00"
    Then I should see a confirmation that the delivery has been created and receive its identifier
  ...
  Scenario: Delivery creation fails when the shipping time exceeds the scheduling horizon
    When I create a delivery with weight "2" kg, pickup location "via Emilia, 9", destination "via Veneto, 5" to ship in "30" days
  ...
  Scenario: Delivery creation fails with a shipping time in the past
    When I create a delivery with weight "2" kg, pickup location "via Emilia, 9", destination "via Veneto, 5" to ship on "2020-01-01" at "10:00"
  ...
  Scenario: Delivery rejected because the package is too heavy for any drone
    ...
    When I create a delivery with weight "8" kg, pickup location "via Emilia, 9", destination "via Veneto, 5" to ship immediately
  ...
  Scenario: Delivery rejected because it cannot meet the maximum delivery time
    When I create a delivery with weight "2" kg, pickup location "via Emilia, 9", destination "via Roma, 200" to ship immediately within "5" minutes
  ...
  Scenario: Delivery creation fails with an invalid address
    When I create a delivery with weight "2" kg, pickup location "xxxxx", destination "via Veneto, 5" to ship immediately
